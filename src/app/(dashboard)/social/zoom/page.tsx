...
export default function ZoomPage() {
  ...
  const fetchMeetings = useCallback(async () => {
    ...
    // Using call_log or video type as proxies for meeting events in Zoom
    let query = supabase
    ...
    },
    {
      ...
      render: (row) => (
        <span className="font-semibold text-sm text-slate-800 dark:text-slate-100 flex items-center gap-2">
          ...
          {row.group_name || row.body || "Instant Meeting"}
        </span>
      )
    },
    {
      key: "participants",
      header: "Participants / Host",
      render: (row) => (
        <span className="text-sm text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
          <Users className="h-3.5 w-3.5" />
          {row.sender_name || row.receiver_name || "Multiple Participants"}
        </span>
      )
    },
    {
  ...
  if (!selectedDeviceId) {
    return (
      <div className="p-8 pb-20 animate-in fade-in">
        <PageHeader title="📹 Zoom" />
        <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed border-2">
          <AlertCircle className="h-10 w-10 text-amber-500 mb-4" />
          <h3 className="text-lg font-semibold mb-2">No Device Selected</h3>
          <p className="text-slate-500 max-w-sm">Select a device to view Zoom meetings and chats.</p>
        </Card>
      </div>
    )
  }
    ...
    <div className="space-y-4 animate-in fade-in pb-20">
      <Tabs defaultValue="meetings">
        <div className="flex items-center justify-between mb-4">
          <TabsList className="h-9">
            ...
            <TabsTrigger value="chat" className="text-xs gap-1.5 focus:outline-none">
              <MessageCircle className="h-3.5 w-3.5" /> Zoom Team Chat
            </TabsTrigger>
          </TabsList>
        ...
        <TabsContent value="meetings" className="mt-0 space-y-4">
          <PageHeader
            title="📹 Zoom Meetings"
            description="Zoom conference logs, calls, and instant meeting records."
          />
